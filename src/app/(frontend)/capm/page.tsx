...
import { Check, BookOpen, Clock, Target, Users, Layout, Globe, Award, Briefcase, Mail, Phone, Building2, AlertCircle } from 'lucide-react';
...
// Curriculum card styling is fixed by position; only the text/percent is editable.
const CURRICULUM_STYLE = [
    { wrap: "bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow", icon: BookOpen, iconClass: "text-accent", title: "text-primary", badge: "bg-accent/20 text-primary", body: "text-slate-600" },
    ...
    { wrap: "bg-slate-800 text-white p-6 rounded-2xl border border-slate-700 shadow-sm", icon: Briefcase, iconClass: "text-accent", title: "text-white", badge: "bg-slate-700 text-slate-300", body: "text-slate-300" },
];
...
export default async function CapmPage() {
    ...
    const eduCardItems = listArr('eduCardItems', K.eduCardItems);
    const pmEduCardItems = listArr('pmEduCardItems', K.pmEduCardItems);
                    ...
                    <section id="details" className="grid md:grid-cols-2 gap-16 items-center">
                        <div>
                            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-accent/20 text-primary font-bold text-2xl md:text-3xl mb-6">
                                <Award size={16} /> {f('detailsBadge', K.detailsBadge)}
                            </div>
                    ...
                    <section className="bg-white rounded-3xl border border-slate-100 shadow-sm p-10 md:p-14">
                        ...
                        <div className="grid md:grid-cols-2 gap-6 mb-10">
                            <div className="rounded-2xl border-2 border-accent/30 bg-accent/5 p-8 flex flex-col gap-5">
                                ...
                                <div>
                                    <h3 className="text-xl font-black text-primary mb-1">{f('eduCardTitle', K.eduCardTitle)}</h3>
                                    <p className="text-slate-500 text-sm font-medium">{f('eduCardSubtitle', K.eduCardSubtitle)}</p>
                                </div>
                                <ul className="space-y-3">
                                    {eduCardItems.map((req, i) => (
                            ...
                            <div className="rounded-2xl border-2 border-primary/20 bg-primary/5 p-8 flex flex-col gap-5">
                                ...
                                <div>
                                    <h3 className="text-xl font-black text-primary mb-1">{f('pmEduCardTitle', K.pmEduCardTitle)}</h3>
                                    <p className="text-slate-500 text-sm font-medium">{f('pmEduCardSubtitle', K.pmEduCardSubtitle)}</p>
                                </div>
                                <ul className="space-y-3">
                                    {pmEduCardItems.map((req, i) => (
                                        <li key={i} className="flex items-start gap-3 text-slate-700 font-medium text-sm">
                                ...
                                {curriculum.map((item, i) => {
                                    ...
                                    return (
                                        <div key={i} className={s.wrap}>
                                            ...
                                            <p className={`${s.body} leading-relaxed text-sm`}>
                                                {item.desc}
                                            </p>
                                        </div>
                                    );
