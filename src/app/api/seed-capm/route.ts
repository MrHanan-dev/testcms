...
/** Seed the CAPM page global from CAPM_CONTENT. Dev-only, secret-guarded, ?force=1.
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const existing = await payload.findGlobal({ slug: "capmPage" });
    ...
    const heroBadgeImage = await uploadImage("certifications", "capm.webp", "CAPM Certification Badge");

    const k = CAPM_CONTENT;
    ...
    await payload.updateGlobal({
      ...
      data: {
        ...
        eduCardTitle: k.eduCardTitle, eduCardSubtitle: k.eduCardSubtitle, eduCardItems: textList(k.eduCardItems),
        pmEduCardTitle: k.pmEduCardTitle, pmEduCardSubtitle: k.pmEduCardSubtitle, pmEduCardItems: textList(k.pmEduCardItems),
        ...
        registerCardHeading: k.registerCardHeading, registerCardPara: k.registerCardPara, registerCardButton: k.registerCardButton,
    ...
    console.error("[seed-capm] failed:", (err as Error).message);
